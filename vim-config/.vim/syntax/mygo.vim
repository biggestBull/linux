...
if !exists("go_highlight_interfaces")
  let go_highlight_interfaces = 1
endif
...
if go_highlight_structs != 0
  syn match goStruct                /\([=,(]\s*\|&\)\@<=\w\+\({\)\@=/
"  syn match goStructDef             /\(type\s\+\)\@<=\w\+\(\s\+struct\s\+{\)\@=/
endif

" Interface;
if go_highlight_interfaces != 0
"  syn match goInterfaceDef             /\(type\s\+\)\@<=\w\+\(\s\+interface\s\+{\)\@=/
endif

hi def link     goStruct            diyType
" hi def link     goStructDef         diyType
" hi def link     goInterfaceDef      diyType
